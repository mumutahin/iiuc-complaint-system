        ...
        return (
          <li key={`${log.timestamp}-${i}`} className="relative flex gap-3 pb-6 last:pb-0">
            ...
            <div>
              ...
              {log.performedBy && <p className="text-xs text-ink/50 dark:text-white/40">by {log.performedBy}</p>}
              {log.details && <p className="text-xs italic text-ink/45 dark:text-white/35">{log.details}</p>}
